...
async def test_icewash_seed_is_idempotent(db_session_factory):
    ...
    async with db_session_factory() as session:
        ...
        strategies = next(row for row in rows if row.name == "get_whatif_strategies")
        assert strategies.execution["timeout_s"] == 30
        assert strategies.input_schema["additionalProperties"] is False

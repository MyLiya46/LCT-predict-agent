...
async def handle(input_data: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    missing = [key for key in ("system_forecast_number", "strategy_id") if input_data.get(key) in (None, "")]
    if missing:
        return need_input(*missing)
    rows = await build_whatif_rows(context, input_data["system_forecast_number"], input_data.get("category"))
    body = {"strategy_id": input_data["strategy_id"], "rows": rows}
    ...
    return {"response_type": "simulation", "task_id": task_id, "envelope": payload}

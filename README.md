# LCT-predict-agent · 通用 AI Agent 平台（v0.1.2）

内部人机对话智能助手：自然语言查询销售数据 + 销售预测，全链路可追溯，双端 RBAC（用户端/管理员端），Docker 一键部署。

当前版本已由“独立能力演示”升级为可联调的业务平台：统一 PostgreSQL、Alembic 和领域模型，打通预测、归因、What-if、策略优化与聊天；补齐双通道认证、RBAC、管理端、LLM 网关、SSE、沙箱和审计追踪。Agent 支持结构化工作流及六个月基线，统一价格、成本、毛利、覆盖率和库存状态口径；前端新增预测归因图、策略看板和复合结果展示，并完善模型 relay、契约、测试与端到端验收。
